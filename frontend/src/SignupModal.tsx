...
export const SignupModal = ({ onClose, onSwitchToLogin }: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addNotification({ message: 'Welcome! A verification email has been sent to your inbox.', type: 'info' });
      // onSuccess is handled by AuthContext listener
    } catch (err: any) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center animate-fade-in" onClick={onClose}>
      <div className="bg-brand-surface rounded-lg shadow-xl p-6 w-full max-w-sm m-4 relative" onClick={(e) => e.stopPropagation()}>
        ...
        <h2 className="text-xl font-bold text-center text-brand-text-primary mb-4">Create Account</h2>
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            <label htmlFor="email-signup" className="block text-sm font-medium text-brand-text-secondary mb-1">Email</label>
            <input 
              type="email" 
              id="email-signup" 
              value={email}
           ...
           <div className="mb-4">
            <label htmlFor="password-signup" className="block text-sm font-medium text-brand-text-secondary mb-1">Password</label>
            <input 
              type="password" 
              id="password-signup" 
              value={password}
